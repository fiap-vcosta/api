...
4. Salve. Na janela da demo, filtre `env:prod` / `service:api` se os widgets vierem vazios nos primeiros minutos.
...
- Métricas de negócio: logs `OS {OrdemServicoId} movida para o status {Status}` (`service:api`, facets `@Status` / `@OrdemServicoId`).
- Janela: 5–10 minutos.
