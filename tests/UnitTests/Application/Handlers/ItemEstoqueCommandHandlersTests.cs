...
using Application.ItemEstoque.Commands.Handlers;
...
public class CreateItemEstoqueCommandHandlerTests
{
    ...
    public async Task Handle_CreatesItemEstoque_WhenCommandIsValid()
    {
        ...
        _mockRepository.Setup(r => r.CreateAsync(It.IsAny<ItemEstoque>()))
            .Callback<ItemEstoque>(item => item.Id = 1)
        ...
        _mockRepository.Verify(r => r.CreateAsync(It.IsAny<ItemEstoque>()), Times.Once);
    }
    ...
    public async Task Handle_ThrowsInvalidOperationException_WhenCodigoAlreadyExists()
    {
        ...
        _mockRepository.Setup(r => r.GetByCodigoAsync(command.Codigo))
            .ReturnsAsync(new ItemEstoque { Id = 2, Codigo = command.Codigo, Nome = "Outro" });
...
public class UpdateItemEstoqueCommandHandlerTests
{
    ...
    public async Task Handle_UpdatesItemEstoque_WhenItemExists()
    {
        ...
        _mockRepository.Setup(r => r.GetByIdAsync(1))
            .ReturnsAsync(new ItemEstoque { Id = 1, Codigo = "ITM-001", Tipo = ItemTipo.Peca, Nome = "Filtro", UnidadeMedida = UnidadeMedida.Unidade, PrecoVenda = 50.00m, Saldo = 10.000m, SaldoReservado = 1.000m });
        _mockRepository.Setup(r => r.GetByCodigoAsync(command.Codigo))
            .ReturnsAsync((ItemEstoque?)null);
        _mockRepository.Setup(r => r.UpdateAsync(It.IsAny<ItemEstoque>()))
            .Returns(Task.CompletedTask);
        ...
        _mockRepository.Verify(r => r.UpdateAsync(It.IsAny<ItemEstoque>()), Times.Once);
    }
    ...
    public async Task Handle_ThrowsKeyNotFoundException_WhenItemDoesNotExist()
    {
        ...
        _mockRepository.Setup(r => r.GetByIdAsync(999)).ReturnsAsync((ItemEstoque?)null);
...
public class DeleteItemEstoqueCommandHandlerTests
{
    ...
    {
        _mockRepository.Setup(r => r.GetByIdAsync(1))
            .ReturnsAsync(new ItemEstoque { Id = 1, Codigo = "ITM-001" });
